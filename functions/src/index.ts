...
import {profesionalStats} from "./profesionalStats";
...
const profesional = new profesionalStats();
...
// Retorna el usuario traido por el UID
app.get("/getUsuario", async (req, res) => {
  let usuario = await db.doc(`/users/${req.query.uid}`).get();
  res.json(usuario.data());
});

app.post("/anularSesionProfesional", async (req, res) => {
  console.log(req.body.pacienteId);
  console.log(req.body.especialidad);
  console.log(req.body.idProfesional);
  console.log(req.body.idEvento);


  await notify.horaAnuladaXprofesional(db, req.body.pacienteId, req.body.idEvento);
  await gestion.anularHoraXprofesional(db, req.body.idEvento, req.body.especialidad, req.body.idProfesional);
  res.json("ok");
});


app.get("/pacientesAgendadosCount", async (req, res) => {
  const uidProfesional = req.query.uidProfesional as string;
  const especialidad = req.query.especialidad as string;
  res.json( await profesional.pacientesAgendadosCount(db, uidProfesional, especialidad));
});

app.get("/getSesionesProfesional", async (req, res) => {
  const uidProfesional = req.query.uidProfesional as string;
  res.json(await profesional.getSesionesProfesional(db, uidProfesional));
});

app.get("/getSesionesProfesionalTodas", async (req, res)=>{
  const uid = req.query.uid as string;
  gestion.getSesionesXProfesional(db, uid).then((sesiones: any) => {
    res.json(sesiones);
  });
});
