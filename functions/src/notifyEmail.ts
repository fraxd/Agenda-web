...
export class notifyEmail {
  async aperturaAgendaNotify(db: admin.firestore.Firestore, uid: string,
      fechaPartida: Date, fechaUltima: Date) {
    // let temp = await db.doc(`users/${uid}`).get();
    const fechaInicio = fechaPartida.toLocaleDateString();
    const fechaFin = fechaUltima.toLocaleDateString();
    // let user = temp.data();
  ...
  async sendEmailNewReserva(db: admin.firestore.Firestore, uidProfesional: string, uidPaciente: string, idEvento: string, especialidad: string) {
    const tempPaciente = await db.doc(`users/${uidPaciente}`).get();
    const paciente = tempPaciente.data();
    const tempProfesional = await db.doc(`users/${uidProfesional}`).get();
    const profesional = tempProfesional.data();
    const tempEvento = await db.doc(`sessions/${especialidad}/${uidProfesional}/${idEvento}`).get();
    const evento = tempEvento.data();
    const fecha = new Date(evento?.start).toLocaleDateString();
    const time = new Date(evento?.start).toLocaleTimeString();
  ...
  async emailLinkZoom(db: admin.firestore.Firestore, uidReserva: string, join_url: string) {
    ...
    const date = timeStart.toLocaleDateString();
    const hour = timeStart.toLocaleTimeString();
  ...
  async horaAnuladaXprofesional(db: admin.firestore.Firestore, uidPaciente: string, uidEvento: string) {
    ...
    const fecha: string = new Date(docSesionReserva.timeStart).toLocaleDateString();
    const nombreProfesional: string = docSesionReserva.nombreProfesional;
    db.collection("mail").add({
      toUids: [uidPaciente],
      to: "fraxd98@gmail.com", // Este correo es de pruebas para que se me envie una copia siempre
      message: {
        subject: "Tu sesion a sido Anulada", // A continuacion viene el html del mensaje es bastante largo asi que mejor no revisarlo no es muy util
        html: ` 
                <!doctype html>
                <html>
                  <head>
                                ...
                                <td class="wrapper">
                                  <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                                    <tr>
                                      <td>
                                        <p>Hola!,</p>
                                        <p>Lamentamos comentarte que el profesional ${nombreProfesional} a tenido que cancelar tu cita con el para el dia ${fecha}</p>
                                        <p> Desde ya te pedimos nuestras mas humildes disculpas y esperamos que no vuelva a suceder. </p>
                                        <p> Te invitamos a re agendar tu cita dentro de nuestro stio web. </p>
                                        <table role="presentation" border="0" cellpadding="0" cellspacing="0" class="btn btn-primary">
                                          <tbody>
                                            <tr>
                                              <td align="left">
                                                <table role="presentation" border="0" cellpadding="0" cellspacing="0">
                                                  <tbody>
                                                    <tr>
                                                      <td> <a href="https://proyecto-agenda-web-a1177.firebaseapp.com/" target="_blank">Re Agenda tu hora.</a> </td>
                                                    </tr>
                                                  </tbody>
                                                </table>
                                              </td>
